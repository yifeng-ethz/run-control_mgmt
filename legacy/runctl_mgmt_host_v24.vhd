...
architecture rtl of runctl_mgmt_host is 
    ...
    -- Local MM write -> run-control injection path.
    --   write 0x00000000 : flush log FIFO
    --   write /= 0       : inject one local command
    --                      [31:8] payload bytes (MSB-first)
    --                      [7:0]  command byte
    signal local_cmd_mm_word                : std_logic_vector(31 downto 0);
    signal local_cmd_mm_req_toggle          : std_logic;
    signal local_cmd_ack_toggle_mm_sync     : std_logic_vector(1 downto 0);
    signal local_cmd_mm_busy                : std_logic;
    signal local_cmd_word_sync_ff0          : std_logic_vector(31 downto 0);
    signal local_cmd_word_sync_ff1          : std_logic_vector(31 downto 0);
    signal local_cmd_req_toggle_lvds_sync   : std_logic_vector(1 downto 0);
    signal local_cmd_req_toggle_lvds_seen   : std_logic;
    signal local_cmd_capture_pending_lvds   : std_logic;
    signal local_cmd_ack_toggle_lvds        : std_logic;
    signal local_cmd_pending_lvds           : std_logic;
    signal local_cmd_consume_lvds           : std_logic;
    signal local_cmd_word_lvds              : std_logic_vector(31 downto 0);
...
begin
    local_cmd_mm_busy <= local_cmd_mm_req_toggle xor local_cmd_ack_toggle_mm_sync(1);
    
    proc_local_cmd_cdc : process (lvdspll_clk)
    begin
        if rising_edge(lvdspll_clk) then
            local_cmd_req_toggle_lvds_sync(0) <= local_cmd_mm_req_toggle;
            local_cmd_req_toggle_lvds_sync(1) <= local_cmd_req_toggle_lvds_sync(0);
            local_cmd_word_sync_ff0           <= local_cmd_mm_word;
            local_cmd_word_sync_ff1           <= local_cmd_word_sync_ff0;
            
            if (lvdspll_reset = '1') then
                local_cmd_req_toggle_lvds_seen <= '0';
                local_cmd_capture_pending_lvds <= '0';
                local_cmd_ack_toggle_lvds      <= '0';
                local_cmd_pending_lvds         <= '0';
                local_cmd_word_lvds            <= (others => '0');
            else
                if (local_cmd_consume_lvds = '1') then
                    local_cmd_pending_lvds    <= '0';
                    local_cmd_ack_toggle_lvds <= not local_cmd_ack_toggle_lvds;
                end if;
                
                if (local_cmd_capture_pending_lvds = '1') then
                    local_cmd_word_lvds            <= local_cmd_word_sync_ff1;
                    local_cmd_pending_lvds         <= '1';
                    local_cmd_capture_pending_lvds <= '0';
                end if;
                
                if (local_cmd_req_toggle_lvds_sync(1) /= local_cmd_req_toggle_lvds_seen) then
                    local_cmd_req_toggle_lvds_seen <= local_cmd_req_toggle_lvds_sync(1);
                    local_cmd_capture_pending_lvds <= '1';
                end if;
            end if;
        end if;
    end process;
    ...
    begin 
        if rising_edge(lvdspll_clk) then 
            ...
            local_cmd_consume_lvds  <= '0';
            case recv_state is 
                when IDLE => 
                    if (local_cmd_pending_lvds = '1') then
                        local_cmd_consume_lvds          <= '1';
                        synclink_recv.run_command       <= local_cmd_word_lvds(7 downto 0);
                        synclink_recv.timestamp         <= std_logic_vector(gts_counter);
                        synclink_recv.payload_len       <= get_payload_length('0' & local_cmd_word_lvds(7 downto 0));
                        ...
                        case to_integer(unsigned(local_cmd_word_lvds(7 downto 0))) is
                            when CMD_RUN_PREPARE =>
                                synclink_recv.payload(2)     <= local_cmd_word_lvds(31 downto 24);
                                synclink_recv.payload(1)     <= local_cmd_word_lvds(23 downto 16);
                                synclink_recv.payload(0)     <= local_cmd_word_lvds(15 downto 8);
                                synclink_recv.run_number(2)  <= local_cmd_word_lvds(31 downto 24);
                                synclink_recv.run_number(1)  <= local_cmd_word_lvds(23 downto 16);
                                synclink_recv.run_number(0)  <= local_cmd_word_lvds(15 downto 8);
                            when CMD_RESET =>
                                synclink_recv.payload(1)            <= local_cmd_word_lvds(23 downto 16);
                                synclink_recv.payload(0)            <= local_cmd_word_lvds(15 downto 8);
                                synclink_recv.reset_assert_mask(1)  <= local_cmd_word_lvds(23 downto 16);
                                synclink_recv.reset_assert_mask(0)  <= local_cmd_word_lvds(15 downto 8);
                            when CMD_STOP_RESET =>
                                synclink_recv.payload(1)             <= local_cmd_word_lvds(23 downto 16);
                                synclink_recv.payload(0)             <= local_cmd_word_lvds(15 downto 8);
                                synclink_recv.reset_release_mask(1)  <= local_cmd_word_lvds(23 downto 16);
                                synclink_recv.reset_release_mask(0)  <= local_cmd_word_lvds(15 downto 8);
                            when CMD_ADDRESS =>
                                synclink_recv.payload(1)       <= local_cmd_word_lvds(23 downto 16);
                                synclink_recv.payload(0)       <= local_cmd_word_lvds(15 downto 8);
                                synclink_recv.fpga_address(1)  <= local_cmd_word_lvds(23 downto 16);
                                synclink_recv.fpga_address(0)  <= local_cmd_word_lvds(15 downto 8);
                            when others => 
                                null;
                        end case;
                        
                        recv_state <= LOGGING;
                    elsif (asi_synclink_error(2) = '0') then -- lvds is trained
    ...
    begin
        if (rising_edge(mm_clk)) then 
            if (mm_reset = '1') then
                ...
                local_cmd_mm_word           <= (others => '0');
                local_cmd_mm_req_toggle     <= '0';
                local_cmd_ack_toggle_mm_sync<= (others => '0');
            else 
                local_cmd_ack_toggle_mm_sync(0) <= local_cmd_ack_toggle_lvds;
                local_cmd_ack_toggle_mm_sync(1) <= local_cmd_ack_toggle_mm_sync(0);
                -- default 
                log_fifo_rdreq              <= '0';
                ...
                case read_log_fifo_state is 
                    ...
                    when IDLE =>
                        ...
                        if (avs_log_write = '1') then 
                            if (avs_log_writedata = x"00000000") then 
                                read_log_fifo_state         <= FLUSH;
                                avs_log_waitrequest         <= '0';
                            elsif (local_cmd_mm_busy = '0') then
                                local_cmd_mm_word           <= avs_log_writedata;
                                local_cmd_mm_req_toggle     <= not local_cmd_mm_req_toggle;
                                avs_log_waitrequest         <= '0';
                            end if;
                        end if;
                    when POP_LOG =>
